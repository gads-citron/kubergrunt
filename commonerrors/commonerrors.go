...
func (err ImpossibleErr) Error() string {
	return fmt.Sprintf(
		"You reached a point in kubergrunt that should not happen and is almost certainly a bug. Please open a GitHub issue on https://github.com/gads-citron/kubergrunt/issues with the contents of this error message. Code: %s",
		string(err),
	)
}
